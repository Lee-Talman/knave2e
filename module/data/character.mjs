...
export default class Knave2eCharacter extends Knave2eActorType {
  ...
  _deriveLevel() {
    if (game.settings.get("knave2e", "automaticLevel") === false) {
      ...
      const validLevels = Object.entries(
        JSON.parse(game.settings.get("knave2e", "xpPerLevel")),
      )
  ...
  _deriveSlots() {
    if (game.settings.get("knave2e", "automaticSlots") === true) {
      this.slots.max =
        ...
        (this.wounds.max - this.wounds.value);
    }
  ...
  _deriveBrokenWeapons() {
    const weapons = this.parent.items.contents.filter(
    ...
    weapons.forEach((weapon) => {
      if (weapon.system.brokenQuantity >= weapon.system.quantity) {
        weapon.update({
          "system.broken": true,
          "system.brokenQuantity": weapon.system.quantity,
        });
      } else {
  ...
  async getRestData() {
    ...
    if (type === "standard") {
      ...
      ui.notifications.warn(
        "No rest type selected. Defaulting to standard rest...",
      );
      return update;
    }
  }
