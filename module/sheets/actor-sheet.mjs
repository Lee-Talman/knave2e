...
export default class Knave2eActorSheet extends ActorSheet {
  ...
  async _onItemToggle(event) {
    ...
    switch (a.dataset.action) {
      case "break":
        ...
        return item.update({
          "system.brokenQuantity": newBreakQuantity,
          "system.broken": !item.system.broken,
        });
      case "cast":
        ...
        return item.update({
          "system.castQuantity": newCastQuantity,
          "system.cast": !item.system.cast,
        });
      case "equip":
  ...
  async _isIdentical(itemData) {
    ...
    switch (itemData.type) {
      case "armor":
        inventory = this.actor.items.filter(
          (existingItem) => existingItem.type === "armor",
        );
        match = inventory.find(
          (existingItem) =>
            ...
            existingItem.system?.relic.isRelic ===
              itemData.system?.relic.isRelic &&
      ...
      case "equipment":
        inventory = this.actor.items.filter(
          (existingItem) => existingItem.type === "equipment",
        );
        match = inventory.find(
          (existingItem) =>
            existingItem.name === itemData.name &&
      ...
      case "lightSource":
        inventory = this.actor.items.filter(
          (existingItem) => existingItem.type === "lightSource",
        );
        match = inventory.find(
          (existingItem) =>
            existingItem.name === itemData.name &&
            ...
            existingItem.system?.brightRadius ===
              itemData.system?.brightRadius &&
      ...
      case "spellbook":
        inventory = this.actor.items.filter(
          (existingItem) => existingItem.type === "spellbook",
        );
        match = inventory.find(
          (existingItem) =>
            existingItem.name === itemData.name &&
      ...
      case "weapon":
        inventory = this.actor.items.filter(
          (existingItem) => existingItem.type === "weapon",
        );
        match = inventory.find(
          (existingItem) =>
            existingItem.name === itemData.name &&
            ...
            existingItem.system?.damageDiceAmount ===
              itemData.system?.damageDiceAmount &&
            existingItem.system?.damageDiceSize ===
              itemData.system?.damageDiceSize &&
            existingItem.system?.damageDiceBonus ===
              itemData.system?.damageDiceBonus &&
      ...
      moveQuantity = await foundry.applications.api.DialogV2.prompt({
        ...
        ok: {
          ...
          callback: (event, button, dialog) =>
            button.form.elements.moveQuantity.valueAsNumber,
        },
        rejectClose: false,
      });
    } else {
